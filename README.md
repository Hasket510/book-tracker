...
- **Каталог и поиск**  
  Страница `/bookList`: запрос к [Google Books API](https://www.googleapis.com/books/v1/volumes), сортировка по новизне (`orderBy=newest`), подгрузка следующих страниц по 10 книг (бесконечный скролл через `IntersectionObserver`). При первом открытии показывается выдача по запросу «случайные книги».
